...
#any NA search method would return no NAs

#Create TSA object with new drop rows
ts_drop_rows_price <- ts(drop_rows_price, frequency=12)
tail(ts_drop_rows_price)

#Notice date end in september and not november!!.. just shows you have missing data but not where

#Create a data vector with all observations from January 2001 to November 2022
#Make sure you use month month steps
# use function seq.Date--> start with from and to is the last date it should be 
#use as date to create it as date object
# by is time interval
Days <- as.data.frame(seq.Date(from=as.Date("2001/01/01"),to=as.Date("2022/11/01"), by ="month"))
colnames(Days) <- "Month" #same column name as OG dataset

#merge with the data with missing rows
# left dataframe is days, your new complete date data frame
# they both have a column "Month"
# using left join means you keep all the "month" observations in Days 
#if anything is missing in Drop rows price it's replaced with NA
new_price <-left_join(Days, drop_rows_price, by="Month")

#You should be able to see NAs for the dates missing in drop_rows_price 
# with new merged dataset you can run the na.approx or ts celan
...
#same function but change the lag from 1 to 12 for differencing 
...
```{r}
#trying arma 1,1 and we know little D is 1
Model1 <- Arima(ts_electricity_price[,"Residential"],
                order=c(1,1,1),seasonal=c(0,1,1),include.drift= FALSE)
print(Model1)

plot_grid(autoplot(Model1$residuals),
...
#autoplot(ts_drop_rows_price[,"Residential"],series="Original")
